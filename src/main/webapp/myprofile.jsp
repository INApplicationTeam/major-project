...
<body class="profile-page">
    ...
    $('#submit').click(function(){
      ...
      function isAnswerEmpty(delta,post)
      {
    	  var contentAns="";
  		for(var i=0;i<delta.ops.length;i++)
  		{
  			var del=delta.ops[i];
  			contentAns = contentAns + del.insert;
  		}
  	
  		if(contentAns.replace(/\n/g, '').trim().length===0)
  		{
  			
  			alert("You Cannot Post Empty "+post+"...!!!!");
  			return false;
  		}
  		return true;
...
{   
    index=x;
    var delansx=document.getElementsByClassName("delans");
    var val=delansx[x].value;
    request=getXmlHttpRequestObject();
    request.onreadystatechange=confirmDelete;
    request.open("post","DeleteMyContent",true);
...
{   
    index=x;
    var delquex=document.getElementsByClassName("delque");
    var val=delquex[x].value;
    request=getXmlHttpRequestObject();
    request.onreadystatechange=confirmDelete;
    request.open("post","DeleteMyContent",true);
    request.setRequestHeader ("Content-Type", "application/x-www-form-urlencoded");
...
{   
    index=x;
    var delblogx=document.getElementsByClassName("delblog");
    var val=delblogx[x].value;
    request=getXmlHttpRequestObject();
    request.onreadystatechange=confirmDelete;
    request.open("post","DeleteMyContent",true);
    request.setRequestHeader ("Content-Type", "application/x-www-form-urlencoded");
...
function confirmDelete()
{
    if(request.readyState===4 && request.status===200){
        ...
        {
        	editor[i].style.border="0px solid #ccc";
    ...
    for (var k = elems.length - 1; k >= 0; k--) {
        ...
        {
        	editor[i].style.border="1px solid #ccc";
       ...
       <script>
            ...
            if(document.getElementById("blogform").style.display==="none")
            {
            	...
            	document.getElementById("blogform").style.border="1px solid #ccc";
            }
        
            else if(document.getElementById("blogform").style.display="block")
            {
            	document.getElementById("blogform").style.display="none"; 
            	document.getElementById("blogform").style.border="0px solid #ccc";
            }
            //quill.enable(false);
            }
